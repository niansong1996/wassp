...
class QAProgrammingEnv(Environment):
  ...
  def __init__(self, en_vocab, de_vocab,
    ...
    self.sketch_constraint = None
    self.reset()

  def set_sketch_constraint(self, sketch):
    self.sketch_constraint = sketch

  def sketch_pruning(self, valid_tokens, current_token):
    if self.sketch_constraint is None:
        return valid_tokens

    if current_token == '(' and len(self.sketch_constraint) > 0:
        chosen_token = self.sketch_constraint.pop(0)
        assert(chosen_token in valid_tokens)
        pruned_tokens = [chosen_token]
    elif current_token == ')':
      if len(self.sketch_constraint) == 0:
        chosen_token = '<END>'
      else:
        chosen_token = '('
      assert(chosen_token in valid_tokens)
  ...
  def step(self, action, debug=False):
    ...
    current_token = self.de_vocab.lookup(self.valid_actions[action], reverse=True)
    valid_tokens = self.sketch_pruning(self.interpreter.valid_tokens(), current_token)
  ...
  def clone(self):
    ...
    new.sketch_constraint = None if self.sketch_constraint is None else self.sketch_constraint[:]
    return new
