...
def refine_table_type(table):
    '''
    check every text column to see if we can parse it to numerical column
    '''

    success_refined = False

    for i, type in enumerate(table['types']):
        if type == 'text': # potential mis-typed column
            unparseable_cell = False

            for j, _ in enumerate(table['rows']):
                try:
                    float(parse_decimal(table['rows'][j][i]))
                except NumberFormatError:
                    unparseable_cell = True
                    break

            if not unparseable_cell:
                # change the type of the header and parse all cells to numerical ones
                success_refined = True

                table['types'][i] = 'real'
                for j, _ in enumerate(table['rows']):
                    table['rows'][j][i] = float(parse_decimal(table['rows'][j][i]))

    return success_refined
    ...
    for i, row in enumerate(rows):
        ...
        for j, val in enumerate(row):
            if table['types'][j] == 'real':
                ...
                processed_vals.append([to_unicode(normalize(val.lower()))])
...
def main(unused_argv):
    ...
    refined_table_count = 0
    kg_dict = {}
    for tb_dict in [dev_table_dict, train_table_dict, test_table_dict]:
        for i, (k, v) in enumerate(tb_dict.iteritems()):
            if i % 1000 == 0:
                print i
            refined_table_count += refine_table_type(v)
            kg_dict[k] = table2kg(v)

    print('%d table changed after refinement out of total of %d' % (refined_table_count, len(kg_dict)))
